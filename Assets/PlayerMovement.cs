...
public class PlayerMovement : MonoBehaviour {
    ...
    public void Update() {
         ...
         if (insideBoat && !exitBox.GetComponent<ExitBoxConnect>().jumpOutBoat) {
            ...
            if(exitBox.GetComponent<ExitBoxConnect>().jumpOutBoat && insideBoat && !exitedBoat){
                transform.position =  (new Vector3(1.319f,1.838f,0));
                box.transform.position =(new Vector3(1.653f,1.838f,0));
                insideBoat = false;
                exitedBoat = true;
                Debug.Log("land on Island?"); 
            }
            ...
            transform.position = Vector3.MoveTowards(transform.position, targetPosition.position, 0.32f);
    ...
    public void MoveBoat() {
        ...
        if (boat != null) {
            targetPositionBoat = boat.transform.position + new Vector3(2*moveDir.x / 30.1f, 2*moveDir.y / 30.1f, 0f);
